...
- **Версия:** 2.18.0 (2025-10-16)
- **Строк кода:** ~3800 (Python, после V2 рефакторинга + evaluate_js fix + animation tuning)
- **Файлов:** 31 Python модулей (+4 после V2, +1 docs, +1 __version__.py)
- **Команд:** 29 инструментов (evaluate_js полностью переработан, анимации оптимизированы)
- **Архитектура:** V2.18 (Roadmap V2 + evaluate_js fix + animation timing)
- **Производительность:** JSON оптимизация 58.8%, thread-safe async CDP
- **Тестировано:** WSL2 Ubuntu 22.04 + Windows 11 + Comet Browser
- **Последнее обновление:** 2025-10-16 (Animation timing optimization via user testing)
...
3. **Для выполнения JS используй `evaluate_js`** - console capture, timeout, auto-save больших результатов
4. **Проверяй логи** - structured logging в формате `[TIMESTAMP] LEVEL [module] message`
5. **При WSL-проблемах** - сначала проверь IP Helper службу
6. **Для отладки JS** - используй `evaluate_js()` вместо `console_command()` (больше возможностей)
7. **Новые команды через @register** - автоматическая регистрация, DI через CommandContext
8. **Используй self.cdp** - thread-safe async wrapper вместо прямого self.tab.Runtime
